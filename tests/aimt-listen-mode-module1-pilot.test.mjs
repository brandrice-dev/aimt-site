...
(function missingAudioState() {
  ...
  manifest.forEach((c) => {
    ...
    const shouldExist = GENERATED_PILOT_CHUNKS.includes(c.chunkId);
    const exists = existsSync(filePath);
    check('M. MISSING AUDIO STATE', c.chunkId + '.mp3 ' + (shouldExist ? 'exists on disk (generated by this pilot)' : 'does NOT exist on disk (not generated — production untouched)'), exists === shouldExist);
