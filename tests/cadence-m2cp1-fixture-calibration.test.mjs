...
(function rubricMatchesRebuildTests() {
  ...
  // Fingerprint advanced twice more by later, separate tasks: rubric-922199df
  // -> rubric-efe55590 (a Module 1 launch-fix task's addition of an M1
  // `errorMessages` block, module-01.md's approved checkpoint network-error
  // copy) -> rubric-e0ea1714 (the Modules 2-11 bulk launch-triage fix pass's
  // correction of m4cp2's drifted question text, "During the crown
  // assessment..." -> module-04.md's approved "During the crown station...").
  // Neither touched a question or rubric outside of what it names. This
  // assertion still proves nothing else drifted: m2cp1's own question/rubric
  // checks above are unaffected, and the fingerprint is re-pinned rather
  // than removed.
  check('REBUILT RUBRIC', 'The full extracted M0..M11 rubric/question object set hashes to the current fingerprint -- only m2cp1\'s content and (later) M1\'s errorMessages addition and M4\'s m4cp2 wording fix changed it; every other checkpoint is untouched',
