...
(function exitBehaviorFix() {
  ...
  check('EXIT FIX', 'wireCheckpoint() passes its checkpoint container as returnFocusEl for every open() invocation (input/button/voice/status/response all share one open closure)', /openCheckpoint\(\{ moduleId, cpId, question: def\.question, system: def\.system, reviewSystem: def\.reviewSystem, label, errorMessage: def\.errorMessage, returnFocusEl: container \}\);/.test(shellSrc));
