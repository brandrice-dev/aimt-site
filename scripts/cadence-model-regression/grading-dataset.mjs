...
export const GRADING_DATASET = [
  ...
  // ── Module 2 — m2cp1 ──
  // Rewritten 2026-08-31 for the Module 2 curriculum rebuild
  // (course-audit-build): the checkpoint's real question/rubric now cover
  // intake-to-treatment planning, the shoulder contact as intentional
  // first touch, and protecting a mostly-quiet, relaxation-first service —
  // superseding the prior late-arrival/consent-before-touch scenario these
  // fixtures used to test. See M2.systems.m2cp1 in headspa-mastery.html.
  { id: 'm2cp1-competent', checkpointId: 'm2cp1', category: 'competent-grammar-errors',
    studentResponse: 'First i check the intake again quick before she come in, i dont start asking her everything all over, i already know if is fragrance free or not, any area to skip, and what format we doing today. When she arrive i tell her where to change, what she can leave on, where her things go, and what happen right after, so she dont have to wonder about none of that. Before the aromatherapy i put my hand light on her shoulder first, this the actual beginning of the hands on part, is on purpose not some random touch. If intake already say fragrance free i just do that, dont bring out no scent options, but if scent is include i offer the three like normal and let her pick. Once we start most of the time i stay quiet, i dont keep asking is this ok or can i continue every five minute, thats already decided from before. I only talk if something real happen, like temperature need check, she seem uncomfortable, we moving to different part, or she ask me something. And if she change her mind halfway or say something feel weird, i stop and adjust right away, the plan dont mean i ignore her in the moment.',
    expectedDecision: 'pass', expectUnsafeFlag: false,
    notes: 'Hits all eight required elements (intake confirm without restarting, removes preventable arrival/prep uncertainty, establishes fragrance/areas/format before treatment, shoulder contact as intentional first-touch anchor, correct handling of the aromatherapy opening either way, mostly-quiet default without repeated re-permissioning, in-service communication reserved for dynamic/real moments, remains responsive to a changed mind) despite heavy grammar/spelling noise, which must not cause a fail.' },
  { id: 'm2cp1-incomplete', checkpointId: 'm2cp1', category: 'incomplete-missing-in-service-communication',
    studentResponse: 'I look over her intake before she gets here so I know her fragrance preference and any areas to skip, and I explain where to change and what to leave on when she arrives. I put a hand on her shoulder before starting the aromatherapy since that\'s the actual beginning of the service, and if she picked a scent I offer the options and let her choose.',
    expectedDecision: 'revise', expectUnsafeFlag: false,
    notes: 'Strong on intake review, removing arrival uncertainty, and the shoulder contact as intentional first touch, but says nothing at all about what happens once treatment begins — no mention of staying quiet, what would prompt in-service communication, or responsiveness to a changed mind. Required elements 6-8 are genuinely absent, not contradicted.' },
  { id: 'm2cp1-unsafe', checkpointId: 'm2cp1', category: 'unsafe-repeated-permission-and-ignored-plan',
    studentResponse: 'During the service I check with her at every step -- can I start the massage now, is it okay if I move to the scalp, does she want me to continue -- because it\'s important to keep asking permission the whole way through, not just once. I also always offer all three scent options no matter what the intake says, so she never feels boxed in. And a nice gentle shoulder touch during the aromatherapy really helps calm her nervous system down before the massage starts, which sets up the whole relaxation response for the rest of the session.',
    expectedDecision: 'revise', expectUnsafeFlag: true,
    notes: 'Trips three explicit "immediately correct" triggers: treats repeatedly re-asking permission for standard, already-established steps as the preferred approach; overrides an established fragrance-free plan regardless of what intake says; and claims the shoulder contact/aromatherapy regulates the nervous system and guarantees a relaxation response.' },
