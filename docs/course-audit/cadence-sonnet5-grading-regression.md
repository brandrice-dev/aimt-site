...
**Status:** **Three consecutive live runs. The first two were invalidated by infrastructure defects; the third (Section 11) completed cleanly — 17/17, 0 infra/parse failures — and surfaced one regression-fixture defect, now corrected.** Run 1 (72.2%, Section 8): a structured-output extraction/parsing bug. Run 2 (64.7% sentinel, Section 10): the grading token budget (400) was too small for Sonnet 5's default adaptive thinking, truncating output before completion (plus one transient 503). Run 3 (94.1%, Section 11): `runStatus: COMPLETE`, every case reached a real decision; the sole mismatch (`m2cp1-competent`) was traced to a mislabeled fixture — its text never actually stated the rationale the real Module 2 rubric requires — not a model defect. Fixture corrected (minimum edit, grammar/style preserved); generic feedback-grounding constraint added so grading feedback can't invent physiological/medical framing on any checkpoint. **No live evidence yet exists confirming the corrected fixture actually passes — that requires a live re-run, not yet performed.** Not promoted.
...
**Date:** 2026-08-27 (Sections 1–9), 2026-08-28 (Sections 10–11).
