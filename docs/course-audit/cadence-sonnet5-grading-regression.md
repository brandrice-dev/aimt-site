...
**Status:** **TWO consecutive live runs, both invalidated by infrastructure defects, neither a real model-quality measurement.** Run 1 (72.2%, Section 8): a structured-output extraction/parsing bug. Run 2 (64.7% sentinel, Section 10): the grading token budget (400) was too small for Sonnet 5's default adaptive thinking, truncating output before completion (plus one transient 503). Both root causes are now fixed (Sections 9 and 10) — the grading call sends `max_tokens: 4096` + `output_config.effort: 'medium'` + explicit adaptive thinking, a malformed/truncated response is now a recoverable evaluator error (never a false pass/revise), transient 5xx gets a small bounded retry, and the harness separates infra/parse failures from model disagreement. **No live evidence yet exists of Sonnet 5's actual grading quality under a complete, correctly-configured request.** Not promoted.
...
**Date:** 2026-08-27 (Sections 1–9), 2026-08-28 (Section 10).
...
**Sections 1–7 below are the pre-live-run record (dry-run validation, then the blocked state before a key existed). Section 8 diagnosed the first live run's parsing bug. Section 9 recorded that fix. Section 10 diagnosed and fixed a second, independent infrastructure defect (grading token budget) that the first fix's own sentinel retest surfaced — it is the current authority on this candidate's grading status.**
