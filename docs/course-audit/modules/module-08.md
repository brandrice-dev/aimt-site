...
## CURRENT MODULE 8 AUTHORITY (read this first — updated August 24, 2026, final curriculum polish pass)
...
- **9 instructional video chapters.** Videos 02–09 installed (real Vimeo IDs, unchanged). Video 01 (Aromatherapy) remains a placeholder pending real footage — this is the one true remaining content blocker to organic completion.
- **Masterclass teaching model: three restrained layers per chapter** — **How to do it** (guidance + adaptation cues), **Why we do it** (a new, dedicated `why` field per chapter, owner-authoritative service-design rationale), **How to communicate it** (the existing Communication cue / Keep the flow quiet / If they ask system). Small mono-label headers (`.sms-section-label`) organize existing content under these three concepts — no new cards, no chapter lengthened beyond what the labels required.
- **Timing model: Core (60-minute) and Extended (90-minute) AIMT reference formats.** The masterclass videos demonstrate the Extended 90-minute reference. Steps 01–02 (Aromatherapy, Client Positioning + Comfort) are the pre-timer opening; the treatment clock starts at Step 03 (Dry Brushing + Hair Play) — Core at 60:00, Extended at 90:00. Per-chapter timing badges are truthful "~X min left" landmarks, not fabricated exact durations. The retired 60/120-minute model is fully gone from Module 8's student-facing copy.
...
- **Status: "Substantially complete — final owner pass deferred."** Remaining true blockers to final approval: (1) real Video 01 footage, (2) final hosted real-video owner verification, (3) final owner desktop/phone manual QA, (4) explicit owner approval. This task removed essentially every other known Module 8 issue; it did **not** and cannot resolve these four.
