...
## CURRENT MODULE 8 AUTHORITY (read this first — updated September 13, 2026, course source freeze pass)

This block is the fast-reference summary of what currently governs Module 8. The full amendment history below remains for record-keeping, but a developer should not need to read all of it to know the current state.

- **All 9 instructional video chapters are real, installed footage.** Video 01 — real footage delivered September 2026, re-titled to its own approved title, **"Opening Rituals + Microscopy"** (was a placeholder titled "Aromatherapy" as of the August 24 amendments below; that content blocker is resolved). No chapter is a placeholder any longer.
- **Masterclass teaching model: three restrained layers per chapter, now sequenced teach-first.** **How to do it** (guidance + adaptation cues) → **Why we do it** (the `why` field, owner-authoritative service-design rationale) → **How to communicate it** (Communication cue / Keep the flow quiet / If they ask) → **Adapt** (when present) → **Notes** (when present) → **Watch for in the demonstration** (a short viewing lens, moved to sit immediately before the video) → **video** → chapter-completion nav + a one-line "what's next." (Course source freeze pass, 2026-09-13 — see the amendment below. Previously "Watch for" and the video sat near the top of the chapter, ahead of most of this teaching content.) Small mono-label headers (`.sms-section-label`) organize this content — no new cards were added.
- **Timing model: Core (60-minute) and Extended (90-minute) AIMT reference formats.** The masterclass videos demonstrate the Extended 90-minute reference. Steps 01–02 (Opening Rituals + Microscopy, Client Positioning + Comfort) are the pre-timer opening; the treatment clock starts at Step 03 (Dry Brushing + Hair Play) — Core at 60:00, Extended at 90:00. Per-chapter timing badges are truthful "~X min left" landmarks, not fabricated exact durations. The retired 60/120-minute model is fully gone from Module 8's student-facing copy.
- **Format-boundary explanation:** "Designed around the service, not the clock" (Section 8.1) — replaces the earlier, more bureaucratic-sounding "Why Core and Extended stop at 60 and 90" card. Teaches that each format is its own deliberately designed structure, not a compressed/stretched version of the other.
- **Canonical Service Timer:** `aimt-service-timer.html` (repo root) — Core 60 / Extended 90, pre-timer Steps 01–02, treatment clock from Step 03, Supabase-entitlement-gated (redirects signed-out/non-entitled visitors to `student-access.html`). Two links from Module 8 (end-of-preview CTA, persistent footer link) plus a dashboard entry on `my-aimt.html`, all pointing to the same canonical file. No second Timer implementation exists.
- **Printable Head Spa Service Maps** — renamed and moved directly below the masterclass/video experience (previously sat after the Timer feature, near the completion card). Presented as a compact, visually secondary `.info-card` with two download actions; the two PNG assets and their information are unchanged.
- **Relaxation-first communication** — intake determines the plan; the service executes the plan; in-service communication manages dynamic comfort and answers questions without repeatedly re-litigating the protocol. Unchanged by this pass.
- **Module 9 handoff corrected.** Module 8's own completion card ("Up next — Module 9") and its own next-module preview text no longer identify Module 9 as "Sanitation & Reset Systems" — they now correctly preview **Module 9 — Checkout, Client Closing & Pricing Strategy**. This is a Module-8-side text correction only; Module 9's own technical content, `MODULE_TITLES[9]`, and the site-wide home-dashboard module list still read "Sanitation & Reset Systems" and were deliberately left untouched — that is Module 9's own structural identity, to be reconciled when Module 9 itself is implemented, not a Module 8 concern.
- **Completion requirement unchanged:** all 9 video chapters complete + `m8cp1` + `m8cp2` both `passed`. No Service Timer/interaction-click/read-percentage requirement.
- **Status: "Substantially complete — final owner pass deferred."** Real Video 01 footage is delivered and installed (resolved). Remaining true blockers to final approval: (1) final hosted real-video owner verification, (2) final owner desktop/phone manual QA, (3) explicit owner approval.
...
> **Amendment — August 24, 2026 (owner-authoritative final video reconciliation).** The owner supplied the final, real finished instructional footage. It consists of **9 instructional video chapters, not 12** — the earlier 12-chapter structure was built against a placeholder assumption before real footage existed and is now superseded wherever it conflicts with this amendment. All references below to "12 video-led instructional chapters," "12 video-player chapter positions," "the remaining 11 chapters," or "Chapter X of 12" are superseded by the 9-chapter architecture in the replacement table under "Approved video chapter architecture" below. The underlying 17-numbered-step practitioner sequence is unchanged; every prior curriculum correction (consent/fragrance rules, the adapted exfoliation framework, the Step 05 massage-claim correction, the Steps 07/12 scope guardrail, the Step 11 steam/heat correction, the Steps 13–17 sensory-and-closing corrections, and "Explain intentionally, not continuously") is preserved and regrouped under whichever of the 9 real video chapters now covers that content — none was dropped. One new chapter, **Video 02 — Client Positioning + Comfort**, has no prior written curriculum counterpart; its guidance is new, minimal, and non-technical (settling the client, confirming comfort and support, consent-based adjustment) rather than a claim carried over from prior copy. **Module 8 completion now requires all 9 video chapters complete, not 12** — see "Completion behavior" below, updated accordingly. Real video installation for 8 of the 9 chapters (all but Video 01 — Aromatherapy, which has no finished footage yet and remains a placeholder) is complete as of this amendment; Video 01 remains the one required chapter blocking genuine Module 8 completion until real footage is delivered and installed.
>
> **Amendment — August 24, 2026 (final communication-cue polish).** Following two prior narrower communication-cue passes, the owner supplied final, exact-wording approved content for all 9 chapters and corrected the underlying teaching model: silence must be taught explicitly, not represented by simply having no cue. **"What you might say" now has two forms**, both rendered inside the same `.sms-teach` card via a `type` field on each `teach` array entry: (1) a direct spoken cue (default, unchanged structure), for consent, comfort, temperature, an unfamiliar sensation, or a transition; (2) `type:'quiet'` — a **"Keep this quiet"** explanation of why narration is unnecessary, plus an **"If they ask"** response the student can give if the client does ask. This is the smallest data-model change that supports the distinction cleanly — no new card system, no clicks, no color-only meaning (both labels are literal readable text). Final content is exact-owner-wording for all 9 chapters; see `00-aimt-current-course-status.md`'s and `implementation-log.md`'s entries for this pass for the full per-chapter KEEP/REVISE table. One additional, narrowly-scoped correction was made while implementing this: Video 03 (Dry Brushing)'s **guidance** text ("introduce the scalp to stimulation... activating circulation") made an unsupported physiological claim never previously flagged in this document's claims corrections — corrected to technique-only language (brush contact, oil distribution) as a direct, minimal consequence of the owner's explicit "do not restore... circulation... stimulation" instruction for this chapter, not a separate curriculum audit. No chapter title, Vimeo ID, video architecture, or completion-gating requirement changed by this amendment.
>
> **Amendment — August 24, 2026 (relaxation-first communication rebuild — supersedes the prior communication-cue amendment above).** The owner clarified that the prior pass overemphasized scripting and repeated in-service consent, and corrected two things: (1) **the scripts are not a running service narration** — they exist so a practitioner already knows the answer to a likely client question and can respond in a sentence or two without breaking service rhythm, not so every step gets narrated aloud; (2) **standard protocol steps are not repeatedly optionalized mid-service.** Neck/shoulder work (Video 06) and hand/forearm massage (Video 08) are established parts of the service format — adaptations or omissions belong at intake, not as a mid-treatment "would you like me to include...?" question, which was corrected in both chapters to a confident transition statement instead. The label set is now **"Communication cue"** (proactive — positioning, temperature, pressure, a real transition, a necessary pre-service choice), **"Keep the flow quiet"** (silence is the default; explains why), and **"If they ask"** (a ready response to a likely question) — same `type:'quiet'` data structure as the prior amendment, `type` unset/`'speak'` entries relabeled from "What you might say" to "Communication cue," no new component. A concise module-level explanation was added to the existing `.m8-principle` block (Section 8.2, before the masterclass) covering both the "prepared answer, not a script" framing and the intake/adaptation relationship — two short paragraphs, not a new instructional section. Video 09's cooling-transition cue no longer offers to skip the step by default (that decision belongs at intake); Video 01's fragrance/touch cue is split into two sequential prompts so touch consent is never implied to follow contact that's already begun. See `00-aimt-current-course-status.md` and `implementation-log.md` for this pass's full per-chapter final wording. Completion requirement, video architecture, Vimeo IDs, and chapter titles are unchanged.
>
> **Amendment — August 24, 2026 (Module 8 final convergence — Core/Extended timing model, canonical Service Timer, service maps).** This is the controlling amendment; sections below that conflict with it are superseded. **CURRENT AUTHORITY — read this first:**
> - **9 video chapters.** Videos 02–09 installed (real Vimeo IDs, unchanged); Video 01 (Aromatherapy) remains a placeholder pending real footage — Module 8 cannot organically complete until it is installed.
> - **The obsolete "1-Hour / 2-Hour" (60/120-minute) format model is retired.** The controlling pacing framework is now **Core Format (60-minute AIMT reference protocol)** and **Extended Format (90-minute AIMT reference protocol)** — `fmt1hr`/`fmt2hr` and the `t-2hr` CSS class were renamed `fmtCore`/`fmtExtended`/`t-extended`; `selectFormat()` now takes `'core'`/`'extended'`. The masterclass videos demonstrate the **Extended 90-minute reference** specifically — stated explicitly in Section 8.1's copy, not left implied.
> - **Steps 01–02 (Aromatherapy, Client Positioning + Comfort) are the pre-timer opening; the treatment clock starts at Step 03 (Dry Brushing + Hair Play)** — Core at 60:00, Extended at 90:00. Chapters 01–02 show a `Pre-Timer Opening` badge instead of a numeric pill; Chapter 03 onward shows `Core: ~X min left` / `Extended: ~X min left` landmarks sourced directly from the two AIMT service-map images (below) — truthful remaining-time landmarks, not fabricated per-chapter durations, since the 9 video chapters and the Timer's own operational blocks are not a forced 1:1 mapping.
> - **The canonical AIMT Service Timer is `aimt-service-timer.html`** (repo root), promoted from the owner-supplied `AIMT-Service-Timer-Module8-aligned.html`. It is a full rebuild: an education/resource page (formats, how-to-read-the-timer, launch) plus an immersive full-screen Timer (Core 60 / Extended 90, pre-timer Steps 01–02, treatment clock from Step 03, pause/resume, back/skip, exit/resume-later, treatment-complete state). It reads Supabase `course_entitlements` client-side (same read-only anon-key + RLS pattern as `my-aimt.html`) and redirects signed-out/non-entitled visitors to `student-access.html` before rendering — the only access-control pattern the current flat-site architecture supports; it is real (visitors without access never see the page content), not decorative. The legacy `AIMT-Service-Timer.html`/`AIMT-Service-Timer-clean.html` prototypes (never part of this repo, found only in the owner's local `~/Downloads`) are superseded — no valuable behavior from them was lost; both used the retired 17-step/60-120 model this amendment replaces.
> - **Two links to the canonical Timer exist in Module 8**, both pointing to `aimt-service-timer.html`: a CTA at the end of the Timer preview ("Open the Full Service Timer →") and a persistent link below the feature ("Open the AIMT Service Timer →"). A third, restrained entry ("AIMT Service Timer") was added to the student dashboard (`my-aimt.html`'s Resources card).
> - **The Module 8 Timer preview** (Steps 01–04) now matches the canonical Timer exactly: Steps 01–02 render as static pre-timer cards (no ticking clock, Next/Skip relabeled, Pause disabled), and the live per-second countdown genuinely begins at Step 03, previewing the **Extended** reference only (Core/Extended selection belongs to the full Timer, not the small preview).
> - **Core and Extended Format Head Spa Service Map images are installed** at `assets/images/course/module-08/module-08-core-format-service-map.png` and `-extended-format-service-map.png`, presented in Module 8 as downloadable "Service Maps" (quick protocol/pacing reference, companion to the masterclass and Timer).
> - **Three communication "If they ask" lines were corrected** to this task's authoritative wording (in both `M8_CHAPTERS` and the canonical Timer's own data, kept in sync): Chapter 01 Aromatherapy gained the "If they ask" response it was previously missing entirely; Chapter 06 Neck/Shoulder's and Chapter 08 Hand/Forearm's "if they ask" answers were corrected away from the weaker "we include it as part of the service" framing to purpose-driven answers; Chapter 09's cooling "if they ask" was tightened to the exact approved wording.
> - **Section 8.1 ("Core and Extended")** was rewritten to teach service-structure differences (what each format scales versus keeps fixed), plus a new restrained "Why Core and Extended stop at 60 and 90" card (shorter/longer services need a deliberately different protocol, not a compressed/stretched one — no fabricated 30/45/120-minute timing was introduced).
> - Everything else in this document (curriculum/claims corrections, "Protect the Flow," Cadence identity, both checkpoints' questions/rubrics, the 9-chapter architecture and Vimeo IDs, completion/gating) is unchanged by this amendment.
>
> **Amendment — August 24, 2026 (broad student-facing coherence pass — governing service-communication model formalized).** Following a systematic, top-to-bottom audit of the entire rendered Module 8 student experience against the relaxation-first model above, the governing service-communication model is now stated explicitly as course doctrine: **intake determines the plan; the service executes the plan; in-service communication manages dynamic comfort (temperature, pressure, positioning, real transitions, immediate safety) and answers client questions — it does not repeatedly re-decide whether each standard step happens.** The audit confirmed the prior two amendments had already resolved this pattern comprehensively across Videos 02–09 (mid-service optionalization, unnecessary narration, meta/authoring language, claims regression, and the Module 9 boundary were all already clean on inspection — see the itemized audit in `implementation-log.md`). **Video 01 (Aromatherapy) was the one chapter still carrying the old pattern** and is corrected here: fragrance sensitivity/intolerance/preference is now taught as an intake-stage determination, made **before** Step 01, not a decision the practitioner discovers live at the client's shoulder; the spoken cue changes from "I have three fragrance options, or we can skip fragrance altogether. Which would you prefer?" to **"I have three scent options for you today. Take a moment with each and tell me which one you're most drawn to,"** with guidance stating that a predetermined fragrance-free path is followed instead when intake has already established one. The separate "touch consent" spoken cue is removed as a repeated re-ask; touch readiness for this first-contact moment is now taught as an expectation already established during intake and covered in depth in Module 2 (cross-referenced, not restated) — Step 01 is where the practitioner puts that understanding into practice, not where consent is requested from scratch. **This is recorded as a judgment call, not a silent decision** — see the implementation-log entry for the full reasoning and an explicit flag that the owner should confirm this reading of "put consent at the correct stage of the client journey" is correct. The Timer preview's Step 01 description/note (`M8_TIMER_PREVIEW_STEPS[0]`) and `MODULE_GUIDE_SYSTEMS[8]` (Cadence) were updated in the same narrow, disclosed way to stay consistent with the corrected framing. Fragrance-free service remains fully valid; the exfoliation adapt-by-degree framework, all scope guardrails, and all prior claims corrections were re-audited and found already compliant, unchanged. No chapter title, Vimeo ID, video architecture, checkpoint question/rubric, or completion-gating requirement changed.

> **Amendment — August 24, 2026 (final Module 8 curriculum polish pass — this is the controlling amendment; see CURRENT MODULE 8 AUTHORITY at the top of this document).** The owner requested a comprehensive, final pre-video-QA polish pass to remove essentially every other known Module 8 issue. Approved and implemented:
> - **The literal "How to do it / Why we do it / How to communicate it" three-layer teaching model is now implemented as restrained labeled sections**, closing out the flag the prior convergence task left open (that task judged the model "already substantively present" but explicitly deferred a labeled restructure for owner confirmation). Each of the 9 chapters gained a new `why` field carrying dedicated, owner-authoritative service-design rationale (why the technique belongs in the service — experiential, operational, continuity, or sensory-contrast value; never fabricated physiology). Existing `guidance`/`adapt` render under a small "How to do it" label; the new `why` content renders under "Why we do it"; the existing Communication cue / Keep the flow quiet / If they ask system renders under "How to communicate it." No new cards, no chapter meaningfully lengthened — a handful of guidance sentences that were already rationale, not technique, were relocated into the new `why` field rather than duplicated.
> - **Section 8.1's format-boundary card was renamed and rewritten**: "Why Core and Extended stop at 60 and 90" → **"Designed around the service, not the clock,"** replacing bureaucratic-sounding language ("aren't covered here") with the approved instructional framing (each format is its own deliberately designed structure; expand only where time genuinely adds value).
> - **Printable Head Spa Service Maps moved and renamed.** The download section (previously titled "Service Maps," positioned after the Timer feature near the completion card) now sits directly below the masterclass/video experience, renamed **"Printable Head Spa Service Maps"** with the approved supporting line and two named download actions. Presented as a single compact, visually secondary `.info-card` — not promoted into a major lesson section. The two PNG assets and their information are unchanged.
> - **Module 8's own completion-card Module 9 handoff corrected** from "Sanitation and reset systems..." to "Checkout, client closing, and pricing strategy..." — see "Approved module identity"'s "Transition to Module 9" note above. Module 9's own technical content, `MODULE_TITLES[9]`, and the site-wide home-dashboard module list were deliberately left untouched — that is Module 9's own structural identity, out of scope for a Module 8 polish pass.
> - **Full polish scan.** One stale internal/admin-facing string was found and corrected (`sms-video-sublabel`'s static "Add Vimeo link in admin" fallback → "Instructional video in production — available soon," matching the actual placeholder copy already used elsewhere). One bureaucratic-sounding line was also found and corrected on the canonical Timer's own resource page (`aimt-service-timer.html`'s "Current AIMT Timer scope" callout, which said "AIMT is not presenting 30- or 45-minute services...") — replaced with the same principled framing as the Module 8 format-boundary card. No stale 12-video, 17-step-count, or 1hr/2hr language was found remaining anywhere in Module 8; no dead buttons, no awkward empty cards, no unsupported claims were found restored.
> - Everything else (curriculum/claims corrections, Core/Extended timing model, canonical Timer, Timer preview, 9-chapter video architecture and Vimeo IDs, checkpoints, Cadence, "Protect the Flow," completion/gating) is unchanged by this amendment.
>
> **Not resolved by this amendment, and not claimed to be:** real Video 01 (Aromatherapy) footage, final hosted real-video owner verification, final owner desktop/phone manual QA, and explicit owner approval. Module 8 status remains **"Substantially complete — final owner pass deferred,"** not `Implemented — manual QA approved`.
>
> **Amendment — September 13, 2026 (course source freeze pass, pre-Listen-Mode-production).** Real Video 01 footage was delivered and installed (owner-supplied, September 2026) — the one remaining content blocker noted above is resolved; Video 01 is re-titled to its own approved title, **"Opening Rituals + Microscopy,"** covering the full opening sequence (changing/arrival, intake completion, scalp microscopy, and the aromatherapy ritual), not aromatherapy alone. Five further corrections, approved and implemented in this pass: (1) **the masterclass chapter template is restructured teach-first** — guidance → why → communication → adapt (when present) → notes (when present) → "Watch for in the demonstration" → video → chapter-completion nav, so the video is now each chapter's demonstration/capstone rather than its opener, and no substantial teaching content sits below the video in any of the 9 chapters; Chapter 1 and Chapter 9's titles and Vimeo IDs are unchanged, and Chapter 9's already-reconciled close (`04faf79`) is preserved verbatim. Next/Previous chapter now also scroll to the new chapter's top with clearance reserved for the sticky lesson-nav header, and move accessibility focus to its heading. (2) **The canonical Service Timer** (`aimt-service-timer.html`) **and Module 8's in-page Timer preview** (`M8_TIMER_PREVIEW_STEPS`) **had their Step 01 identity reconciled** from the stale "Aromatherapy" label to "Opening Rituals + Microscopy," covering intake completion, microscopy, live aromatherapy sampling (or the fragrance-free path), and the intentional shoulder first-touch — Core 60 / Extended 90 timing and the pre-timer/timed-service boundary are unchanged. (3) **`docs/course-audit/listen-mode/module-08-listen-script.md`'s Chapter 9 narration was reconciled** to the real, expanded `M8_CHAPTERS[8]` close (it was still narrating the pre-`04faf79` version), and a synchronization note was added recording the teach → "Watch for" → non-narrated video → next-chapter handoff rule for any future full per-chapter Listen Mode narration. (4) **A Cadence Ask-shell/site-header collision bug was found and fixed** (`assets/js/cadence-shell.js`): the shell's top-clearance offset previously accounted only for the Review Mode banner, not the lesson page's own sticky header, so the shell could render without clearance below it; it now measures both live and is offset/sized accordingly, scroll-position-independent. Out of scope for Module 8 specifically, but touches the same shell every Module 8 checkpoint and Ask Cadence use. (5) **The Module 8 "Adapt" callout's icon was corrected** from a stray "→" to the canonical AIMT orbital mark, matching every other plain (non-warn/non-success) `.key-point` callout on the page — no other occurrence of this component used the stray arrow. No curriculum content, checkpoint question/rubric, completion-gating requirement, or Vimeo ID changed by this amendment.
...
**Transition to Module 9 — corrected August 24, 2026 (final curriculum polish).** The completion card's Module 9 preview text previously read "Sanitation and reset systems..." — stale, per the owner's locked future module order (`00-aimt-current-course-status.md`). It now reads "Checkout, client closing, and pricing strategy...", matching **Module 9 — Checkout, Client Closing & Pricing Strategy**. This is a Module-8-side completion-card text correction only; it does not rename, reorder, or reimplement Module 9 itself.
...
- **All 17 numbered service steps and their existing titles, in existing order** — see "Approved video chapter architecture."
- **AMENDED August 24, 2026 — the owner's final 9 video-chapter positions and their owner-authoritative chapter labels** — see the same section. (Superseded: the earlier 12-video-position structure, built before real footage existed.)
...
Do not rename the module ID, checkpoint IDs, completion-card ID, wrapper ID, progress keys, `STEP_VIDEO_IDS` array, or any of the 9 video-slot element IDs merely to normalize implementation.
...
## Approved video chapter architecture — AMENDED August 24, 2026 (owner-authoritative final reconciliation)

**This section supersedes the original "17-step / 12-video" table below it.** The owner has now delivered the real, final instructional footage: **9 instructional video chapters**, reconciled against the existing 17-numbered-step practitioner sequence rather than forcing a stale 12-position placeholder structure to match real footage that was cut differently. Two related but distinct structures remain, kept explicitly distinguished rather than collapsed:

- **17 numbered practitioner service steps** — the actual sequence of what happens in the service, unchanged in number, order, and title (the written curriculum backbone).
- **9 video-led instructional chapters** — the presentation structure students learn from. Multiple numbered steps are grouped under a single chapter where that is how the owner's real footage is organized; one chapter (Video 02) covers content with no prior numbered-step counterpart.

**Owner-authoritative titles, verbatim, do not rename:**

| Chapter | Step(s) reconciled into it | Owner-authoritative title | Vimeo status |
|---|---|---|---|
| 01 | 01 | Aromatherapy | Placeholder — real footage not yet delivered |
| 02 | *(new — no prior step)* | Client Positioning + Comfort | Installed — `1214280975` |
| 03 | 02 | Dry Brushing and Hair Play | Installed — `1213974160` |
| 04 | 03 | Halo Activation + Wet Massage | Installed — `1213975936` |
| 05 | 04, 05, 06 | Exfoliant + Scalp Massage | Installed — `1213972891` |
| 06 | 07 | Neck and Shoulder Massage | Installed — `1213970537` |
| 07 | 08–10 | Shampoo + Rinse | Installed — `1214855873` |
| 08 | 11, 12 | Deep Conditioning / Hand + Arm Massage | Installed — `1214959378` |
| 09 | 13–15, 16–17 | Final Rinse + Halo Massage | Installed — `1214960268` |

**Reconciliation notes (what moved where, and why):**

- **Video 01 (Aromatherapy)** = old Step 01, unchanged in content. Real footage not yet delivered — stays a placeholder; not counted complete until real footage is installed and genuinely watched to the end.
- **Video 02 (Client Positioning + Comfort)** is new — the owner's real footage covers a distinct positioning/comfort segment that the prior 17-step written curriculum never separately named. Its guidance is deliberately minimal and process-level (settling the client, confirming physical comfort and support, consent-based adjustment) — no physiological or outcome claim was introduced, consistent with the governing claims-correction standard elsewhere in this module.
- **Video 05 (Exfoliant + Scalp Massage)** absorbs old Steps 04 (Exfoliant application), 05 (Scalp massage), and 06 (First rinse) — the rinse is the natural closing beat of this same segment in the real footage. All three steps' guidance, client-language examples, adaptation cues, and the approved exfoliation-by-degree and massage-claim corrections are preserved, not merged away.
- **Video 08 (Deep Conditioning / Hand + Arm Massage)** absorbs old Steps 11 (Conditioning/treatment) and 12 (Hand massage). The prior "2-hour only" title suffix on hand massage no longer applies to the chapter as a whole (1-hour students still see quick-conditioning content); the format distinction is preserved as a note instead.
- **Video 09 (Final Rinse + Halo Massage)** absorbs old Steps 13–15 (Waterfall rinse, Cooling spray, Hot towel) **and** 16–17 (Towel wrap, Close & checkout) — the closing script and the approved "clients remember being seen" correction are preserved in full as this chapter's closing notes, since the real footage's final chapter is also the practitioner sequence's true close. This was the one judgment call in the reconciliation requiring the most interpretation: none of the 9 owner-supplied titles individually name "close and checkout," but folding it into the last video chapter (rather than inventing an unlisted 10th chapter, or dropping the approved closing correction) keeps every prior approved correction intact without contradicting the owner's authoritative 9-title list.

All prior step-specific corrections in "Step-specific approved corrections" below remain controlling; only their chapter grouping changed, not their content.

If the real footage contains distinct subclips within a grouped chapter, the implementation may present those as subclips within that chapter's player — this does not require or authorize renaming the chapter or splitting it into a new top-level chapter position.

<details>
<summary>Superseded — original 17-step / 12-video table (kept for history only, do not implement against this)</summary>
...
**Governing requirement:** one primary masterclass component shows the **active chapter only**. The information around the player (title, watch-for cue, guidance, adaptation cue, continuity) changes with the active chapter. The remaining chapters (8 others, in the current 9-chapter architecture — see "Approved video chapter architecture") are represented in a compact chapter rail/progress indicator, not as additional full video stages rendered on the page.
...
- **No repeated "Chapter X of N" text.** The visible chapter numeral (e.g., "05") is sufficient as the visible current-chapter number; full ordinal position (e.g., "Chapter 5 of 9 — Exfoliant & Scalp Massage" in the current 9-chapter architecture) is still exposed to assistive technology via an `aria-label` on the chapter-identity region, so screen-reader users are not shortchanged by the visual simplification.
...
## Governing service-communication model — added August 24, 2026 (broad coherence pass)

This is the single governing principle behind every communication decision in Module 8 — cited by later sections rather than restated in each one:
...
1. **Intake determines the plan.** Fragrance sensitivity/preference, bodywork restrictions, contraindications, scope limitations, positioning needs, and any protocol step that should be modified or omitted are identified before treatment begins — not discovered live, mid-service.
2. **The service executes the plan.** Once treatment begins, standard protocol steps are performed with confidence, not repeatedly re-offered as new optional decisions ("Would you like me to include...?", "Would you rather skip...?"). Any adaptation was already decided at intake.
3. **In-service communication manages dynamic comfort** — water temperature, pressure, positioning, physical comfort, a meaningful/unexpected transition, or an immediate safety concern — things that can genuinely change moment to moment. The client can always request a change or stop; that is not the same as re-litigating the protocol step by step.
4. **The service is relaxation-first.** Most of it stays quiet. The "Communication cue" / "Keep the flow quiet" / "If they ask" scripts exist primarily so a practitioner already knows the answer to a likely question and can respond in one or two sentences, confidently, without hesitation or rambling, then return attention to the treatment — not to narrate technique as it happens.
...
**AMENDED August 24, 2026, further amended August 24, 2026 (relaxation-first rebuild) — silence is taught explicitly, not by omission, and scripts are prepared answers, not narration.** A chapter where the default should be quiet does not simply have an empty field; it carries a **"Keep the flow quiet"** explanation of why narration is unnecessary here, plus (where a likely question exists) an **"If they ask"** response so the student has an accurate, ready answer without having to think on the spot. As of the relaxation-first rebuild: Video 03 (Dry Brushing), Video 04's post-temperature-check portion, Video 05's exfoliant and rinse portions, Video 07 (Shampoo & Rinse), and Video 09's final-rinse/halo-massage portion all use this quiet form; every other communication moment is a **"Communication cue"** — reserved for genuine proactive needs (positioning, temperature, pressure, a real transition, a necessary pre-service choice), never a running commentary on technique. Established protocol steps (neck/shoulder work, hand/forearm massage) are not repeatedly offered as mid-service opt-ins — see the relaxation-first amendment above. See "Approved video chapter architecture" and this document's communication amendments above.
...
Preserve: IDs `m8cp1` and `m8cp2`; stored passed state; voice input; Enter to submit; Shift+Enter for a new line; Review Mode's unsaved behavior; Module 9 gating only after all 9 video chapters are complete and both checkpoints pass (see "Completion behavior — AMENDED").
...
## Service Timer — AMENDED August 18, 2026, further amended August 19, 2026 (twice), superseded August 24, 2026

**Fourth amendment (August 24, 2026) — the Service Timer is now built, canonical, hosted, and linked.** Everything below this note describing the Timer as a future/deferred/separately-audited tool with "no functional launch control" is historical context only — see the CURRENT AUTHORITY block at the top of this document for the controlling state: canonical file `aimt-service-timer.html`, Core (60)/Extended (90) formats, pre-timer Steps 01–02 / clock starts Step 03, Supabase-entitlement-gated, linked from both the Module 8 preview's end-of-preview CTA and its persistent footer link, and from the student dashboard. The Module 8 embedded preview now covers Steps 01–04 (not 01–03) and previews the Extended reference specifically, with genuine pre-timer/clock-start behavior matching the canonical Timer exactly.
...
- **Chapter position without visible "X of N" (amended August 19, 2026):** since the visible UI no longer repeats "Chapter X of N" text, the full ordinal position is instead exposed via an `aria-label` on the chapter-identity region (e.g., "Chapter 5 of 9 — Exfoliant & Scalp Massage" in the current 9-chapter architecture), so screen-reader users are not shortchanged by the visual simplification. The chapter drawer's toggle button uses `aria-expanded`/`aria-controls`.
...
**Module 9 unlock:** `APP_STATE.canAccessModule(9)` requires `isModuleComplete(8)`, which now requires all 9 video chapters complete **and** both checkpoints passed — not checkpoints alone.
...
**Phase 2 state (updated August 24, 2026):** 8 of the 9 required video chapters are now installed and wired to genuine Vimeo `ended` completion. Video 01 (Aromatherapy) has no finished footage yet and remains a placeholder — no student can complete all 9 required video chapters until it is installed, so Module 8 (and therefore Module 9) cannot yet be organically completed by a real student. This is an accurate, expected Phase 2 state, not a defect.
...
3. **(Amended)** The service-step accordion is no longer the module's primary presentation pattern; Module 8 presents one contained masterclass player showing the active chapter's orientation cue, guidance text, and adaptation cue, with the remaining chapters (8 others, in the current 9-chapter architecture) represented in a chapter rail/progress list, not as additional full video stages — see "Video-led chapter architecture — AMENDED."
...
R6. `isModuleComplete(8)` requires all 9 video chapters complete **and** both checkpoints passed; `canAccessModule(9)` reads this combined state.
...
R13. The chapter list is a collapsed-by-default drawer, secondary to the video; no "Chapter X of N" text is visibly repeated outside the drawer (a bare chapter numeral is visible; full position is exposed via `aria-label`).
...
- The video-led chapter shell (title, cue, player, guidance, adaptation cue, continuity) is specified once with a worked example rather than fully rewritten for all chapters in this document — implementation applies the same shell and re-weighting rule to every chapter in the current 9-chapter architecture, preserving all curriculum content not explicitly corrected above.
