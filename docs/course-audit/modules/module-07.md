...
**Reveal content:** correct classification as a text-labeled badge (not color-only — pairs a glyph and text with the semantic color), a concise explanation (1–2 sentences), a one-line supporting lesson, and a "Next setup" control (reading "Continue" on the fourth card). After the fourth card's reveal, a short static conclusion appears: "The rule to keep: professional setup is judged by function, not by whether every treatment room looks identical." The student then continues naturally into `m7cp1`, which is already the next element in the page.
