...
6. **Correct Section 7.1's visible, student-facing Cadence note.** The current note ("One of the earliest mistakes I made was prioritizing what looked impressive in photos...") is a first-person autobiographical claim rendered directly in the lesson body — a more visible instance of the personal-experience-claim problem than the hidden system prompt, since students read it directly. Cadence may say guidance was built from the instructor's applied experience; it may not claim that experience as its own. **Corrected note (as implemented):** "A common early mistake is prioritizing what looks impressive in photos over what actually feels comfortable to lie in for an hour. Choose the bed your clients will want to come back to — not the one that photographs well." — no first-person claim, guidance framed generically rather than as Cadence's own history.
...
**Post-implementation amendment (August 11, 2026; revised same day to its final visual form).** Owner review of the rendered interaction found the original multi-select "flag every genuine mistake" design taught the wrong lesson: 5 of its 8 conditions were genuine mistakes, so the interaction was weighted toward "most things are wrong" rather than requiring the student to actually evaluate function, and its labels ("Genuine mistake" / "Actually fine") didn't name *why* an answer was right. A first revision corrected this to six balanced text-only scenarios with two labeled choices. A same-day second owner review found that version still too text-heavy for what should be a visually-led judgment exercise — real reference photography had since become available — and requested the interaction be rebuilt around images that carry the teaching, one setup at a time. This entry records that final design; it supersedes the six-scenario text version, which is no longer implemented. The interaction now asks a single explicit instructional question, applied once per card: **"Does this setup need to change, or is it simply a different way of working?"**

**Exact task (final):** four visual judgment cards, shown one at a time (never all four simultaneously), each a real photograph of one setup condition with a concise question and exactly two labeled choices:
...
The four cards are balanced — two "Needs correction," two "Acceptable variation" — each with exactly one defensible answer grounded in this specification:

1. **Cart positioned on the practitioner's left**, active-use items still comfortably reachable — **acceptable variation** (reach-zone organization is about reachability, not a fixed side). Image: `module-07-setup-judgment-01-cart-left.png`.
2. **Active-use items positioned well outside the immediate working area**, requiring the practitioner to repeatedly step away — **needs correction** (violates the within-reach zone). Image: `module-07-setup-judgment-02-items-too-far.png`.
3. **Treatment bed with no armrests**, entry/exit and occipital support otherwise functioning normally — **acceptable variation** (armrests are a labeled preference, not a requirement). Image: `module-07-setup-judgment-03-bed-no-armrests.png`.
4. **Client positioned too far from the head end**, occipital poorly supported, neck extended — **needs correction** (the unsupported position Section 7.4's safety note addresses). Image: `module-07-setup-judgment-04-positioning.png`.

All four images live at `assets/images/course/module-07/`, each with a matching `.webp` derivative; source photography is downscaled proportionally (no aspect-ratio distortion) and framed within the card via CSS `object-fit: cover`, not destructively cropped.

**Choices/actions and reveal behavior:** the student picks one of the two choices on the card's front face; either pick immediately transitions the card to a reveal (back) state — the reveal always states the one correct classification and explains why, regardless of which option the student picked. This is a deliberate design choice, not a defect: the interaction does not grade or compare the student's pick, so there is nothing to get "wrong" in a way that blocks progress — the student is never made to guess again, matching the governing standard that ungraded interactions must not simulate a scored test.

**Reveal content:** correct classification as a text-labeled badge (not color-only — pairs a glyph and text with the semantic color) and a concise explanation (1–2 sentences). Cards 1–3 add a one-line supporting lesson and a single "Next setup" control. **Card 4 has no exit/continue control** — a same-day UX polish pass (August 11, 2026) found a generic "Continue" button there redundant, since `m7cp1` already follows immediately in normal page flow. Card 4's reveal instead shows its explanation directly followed by a "What to remember" closing takeaway ("Professional setup is judged by function — support, reach, comfort, and service flow — not by whether every treatment room looks identical."), inline in the same reveal — no separate conclusion element, no click required to see it. Focus moves programmatically (with `preventScroll`, so the page never jumps) to whichever back face is revealed; because card 4's back face has no interactive control, the next real focusable element in document order is inside `m7cp1` itself, so the student tabs there naturally without ever being trapped in the interaction.

**Transition:** a restrained CSS-only rotateY flip on the card container — no third-party animation library. The visible face is swapped at the transform's 90° (edge-on) midpoint, so front and back are ordinary sibling elements toggled via the `hidden` attribute, never two permanently-present faces relying on `backface-visibility` — whichever face is current is the only one assistive technology ever sees. Under `prefers-reduced-motion: reduce`, the transform is skipped entirely and the swap happens immediately.

**Progression display:** "Setup N of 4" is shown above the active card, for orientation only — it is not a progress-bar write, does not affect `APP_STATE`, and does not gate completion.

**Retry/graded/progress behavior:** unchanged in substance from the governing standard — ungraded, no scoring, no `APP_STATE` write, no persistence across reopen, no completion gate. State (which card is active, front/back) resets on every module open via the same full-markup-replacement mechanism already used elsewhere in Module 7 — no explicit JS reset function is needed.

**Accessibility:** each choice button is a native `<button>` with a card-qualified `aria-label` (since "Needs correction"/"Acceptable variation" repeat identically across all four cards and must remain distinguishable to a screen-reader user navigating by control), grouped under `role="group"` naming the setup number; the reveal explanation is an `aria-live="polite"` region; meaningful `alt` text describes what each photograph shows; visible focus and full keyboard operability are preserved on every control; touch targets meet a comfortable minimum size; no meaning is communicated by color alone anywhere in the card.

**Why distinct from Modules 5 and 6:** Module 5's signature interaction was protocol *adaptation* (choosing a service direction for a given presentation); Module 6's was triage (proceed/modify/refer). Module 7's is setup *judgment* — distinguishing a genuine functional problem from an acceptable variation in how a setup is built, a distinct evaluative skill neither prior module required, now taught primarily through images rather than description.
