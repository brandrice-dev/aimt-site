...
| **Status** | **Substantially complete — final owner pass deferred.** Curriculum, masterclass architecture (three-layer How to do it / Why we do it / How to communicate it teaching model, now explicitly labeled per chapter), 9-video reconciliation, Videos 02–09 installed, Vimeo one-play/replay/mobile-fullscreen UX, communication-cue final polish, the Core/Extended timing model ("Designed around the service, not the clock"), the canonical hosted Service Timer, Timer preview, Printable Head Spa Service Maps (moved directly below the masterclass), checkpoints, Cadence, and a corrected Module 9 completion-card handoff are all done. Deferred: real Video 01 (Aromatherapy), final hosted real-video owner verification, full manual QA, final approval. **NOT** `Implemented — manual QA approved`. |
