...
| Current completion requirement | Both checkpoints must be graded `passed` (no read-percentage minimum, no video-watch-percentage, no interaction-click count, no Service Timer use) |
| **Status** | **Substantially complete — final owner pass deferred.** Curriculum, masterclass architecture, 9-video reconciliation, Videos 02–09 installed, Vimeo one-play/replay/mobile-fullscreen UX, communication-cue final polish, Timer preview, checkpoints, and Cadence are all done. Deferred: real Video 01 (Aromatherapy), final hosted real-video owner verification, full manual QA, final approval. **NOT** `Implemented — manual QA approved`. |
