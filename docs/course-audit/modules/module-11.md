...
- Two-column example (`.grid-2col.m11-ai-compare`, top-aligned, not stretched): left column, **AI output** — the owner-supplied realistic AI scalp-analysis dashboard image (`module-11-ai-scalp-analysis-dashboard.png`), which itself displays "Seborrheic dermatitis — 87%" with a confidence-score UI and its own "AI-generated result. Not a diagnosis." disclaimer — no separate repeated text block, since the image already carries the example; a "View full-size example" link (matching Module 3's established full-size-image-link pattern) sits beneath it, since the course's narrow (680px) content column renders the interface text fairly small at native comparison width. Right column, **Human review** — *"A model confidence score is information generated by the system — not a confirmed diagnosis. It may influence what you look at more closely, but it does not become 'You have seborrheic dermatitis' in your client language."* (unchanged).
- A restrained `+`-marked factor list: `+ Training data`, `+ Image quality / lighting`, `+ Capture conditions`, `+ Populations represented`, `+ Independent validation` — spacing corrected (a dedicated `.m11-shift-list` style replaces the inherited `.body-text` line-height/margin, which had read as loose and left extra blank space below the last item).
