...
`tests/certification-randomization.test.mjs` (27/27), `tests/certification-scoring.test.mjs` (21/21), `tests/certification-attempt-ladder.test.mjs` (12/12) — run via `node tests/<file>.mjs`, dependency-free (Node built-ins + ESM only). `tests/fixtures/certification-fixture-bank.mjs` contains synthetic, clearly-labeled placeholder content (never the raw blueprint's wording, never installed into `content-bank.mjs`). Pre-existing suites (`module-09-migration.test.js`, `module-11-relocation-migration.test.js`) re-run clean — zero regressions.
