...
**Implementation date:** August 26, 2026 (engine); August 26, 2026 (content installation); August 26, 2026 (traceability remediation); August 26, 2026 (visual/UX refactor, this update)
...
**Status:** Module 12 — assessment architecture/content complete; visual/UX refactor complete; pending owner manual QA, migration, and live Cadence environment. NOT manually approved. NOT launch-ready.
...
**OWNER VISUAL QA OF MODULE 12 BEFORE MIGRATION.** With the assessment architecture, content (141/141 traceability-verified), and now the visual/UX refactor all complete, the owner should review the redesigned experience — `scripts/review-module12-bank.mjs --browser` gives a localhost-only way to visually inspect the real installed content through the real, unmodified renderer (regenerate Part I by seed, browse all 12 real cases, all 9 real conversations) without touching production data or requiring the live environment; the existing in-browser Review Mode fixture-state switcher remains available for exercising State A/B/C/D flow and copy. Only after the owner approves the visual/UX pass should the remaining environment steps (run the Supabase migration; provision `ANTHROPIC_API_KEY`) proceed, followed by a full live-environment QA pass (real attempt, live Cadence grading, screen-reader, physical-keyboard, real touch-device) before Module 12's status may become "Implemented — manual QA approved."
